...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        pose = raw_input('Enter the pose you wish Fetch to execute:')
        pose_list = pose.split(',')
        pose = [float(p) for p in pose_list]
        print pose
        ...
        if result:
            ...
            if result.error_code.val == MoveItErrorCodes.SUCCESS:
                rospy.loginfo("Fetch executed given pose")
            else:
